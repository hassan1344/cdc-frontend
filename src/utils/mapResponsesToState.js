  ...
  savedResponses.responses.forEach(response => {
    ...
    switch (response.responseType) {
        ...
        if (response.selectedOption !== null && response.selectedOption !== undefined) {
          if (question.scaleOptions) {
            ...
            if (question.options && question.options[optionIndex]) {
              stateResponses[response.questionId] = question.options[optionIndex].text;
            }
          }
        }
        break;
      ...
      case 'checkbox':
        ...
        } else if (response.selectedOptions && Array.isArray(response.selectedOptions)) {
      ...
      case 'text':
      case 'textarea':
        if (response.selectedValue !== null && response.selectedValue !== undefined) {
          stateResponses[response.questionId] = response.selectedValue;
        }
        break;

      case 'wounds_image':
        if (response.selectedValue && Array.isArray(response.selectedValue)) {
          stateResponses[response.questionId] = response.selectedValue;
        }
        break;
